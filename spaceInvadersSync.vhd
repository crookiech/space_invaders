...
entity spaceInvadersSync is
    port(
        ...
        pixel_x    : out unsigned(10 downto 0);  -- Увеличен до 11 бит для 1280
        pixel_y    : out unsigned(9 downto 0);   -- 10 бит достаточно для 768
...
architecture rtl of spaceInvadersSync is
    ...
    constant H_VISIBLE_1280     : integer := 1280;
    constant H_FRONT_PORCH_1280 : integer := 110;
    constant H_SYNC_PULSE_1280  : integer := 40;
    constant H_BACK_PORCH_1280  : integer := 220;
    constant H_TOTAL_1280       : integer := 1650;

    constant V_VISIBLE_1280     : integer := 720;
    constant V_FRONT_PORCH_1280 : integer := 5;
    constant V_SYNC_PULSE_1280  : integer := 5;
    constant V_BACK_PORCH_1280  : integer := 20;
    constant V_TOTAL_1280       : integer := 750;
    ...
    signal h_count : unsigned(10 downto 0) := (others => '0');  -- 11 бит для 1650
    signal v_count : unsigned(9 downto 0) := (others => '0');   -- 10 бит для 806
...
begin
    ...
    begin
        case resolution is
            ...
            when "10" =>  -- 1280x720
                H_VISIBLE_reg     <= H_VISIBLE_1280;
                H_FRONT_PORCH_reg <= H_FRONT_PORCH_1280;
                H_SYNC_PULSE_reg  <= H_SYNC_PULSE_1280;
                H_BACK_PORCH_reg  <= H_BACK_PORCH_1280;
                H_TOTAL_reg       <= H_TOTAL_1280;
                
                V_VISIBLE_reg     <= V_VISIBLE_1280;
                V_FRONT_PORCH_reg <= V_FRONT_PORCH_1280;
                V_SYNC_PULSE_reg  <= V_SYNC_PULSE_1280;
                V_BACK_PORCH_reg  <= V_BACK_PORCH_1280;
                V_TOTAL_reg       <= V_TOTAL_1280;
    ...
    de_i <= '1' when (to_integer(h_count) < H_VISIBLE_reg and 
                      to_integer(v_count) < V_VISIBLE_reg)
            else '0';
